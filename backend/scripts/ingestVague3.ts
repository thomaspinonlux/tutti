/**
 * Ingest VAGUE 3 — STANDARD (artist+title) — étoffe 4 playlists EXISTANTES
 * depuis backend/data/tutti-remplissage-vague3.csv. Colonnes : slug,playlist,artist,title,level.
 *
 * Cibles (par SLUG, préfixé official-pl-) :
 *   musicals-us · duos-cultes · reggae · country
 *
 * GARDE-FOU : ne crée JAMAIS de playlist (abort si un slug cible manque).
 * Sémantique : skip déjà-présent / reuse catalogue / search YT (drop invalides).
 * difficulty = level. Réversible : rollback JSON compatible funPlaylistsRollback.ts.
 */
...
const CSV = `${ROOT}/backend/data/tutti-remplissage-vague3.csv`;
const STAMP = new Date().toISOString().replace(/[:.]/g, '-');
const ROLLBACK = `${ROOT}/vague3-rollback-${STAMP}.json`;
const THROTTLE_MS = 120;

const TARGET_SLUGS = [
  'official-pl-musicals-us',
  'official-pl-duos-cultes',
  'official-pl-reggae',
  'official-pl-country',
];
...
async function main(): Promise<void> {
  ...
  console.log(`\n=== TOTAL : +${totalAdded} tracks · ${totalEchec} youtube-échec ===`);
