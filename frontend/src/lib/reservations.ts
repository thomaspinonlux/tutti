...
export type StatutReservation = 'DEMANDEE' | 'ACCEPTEE' | 'PAYEE' | 'GRATUITE' | 'REFUSEE' | 'ANNULEE';
...
export const lireReglagesReservation = (): Promise<ReglagesPublics> => api('/api/reservations/reglages');
...
export const annulerReservationGestion = (id: string): Promise<{ ok: boolean; rembourser_dans_stripe: boolean }> =>
...
export function texteCreneau(debut: string, fin: string): string {
  ...
  const jour = d.toLocaleDateString('fr-FR', { timeZone: FUSEAU, weekday: 'long', day: 'numeric', month: 'long' });
  const heure = (x: Date): string => x.toLocaleTimeString('fr-FR', { timeZone: FUSEAU, hour: '2-digit', minute: '2-digit' });
