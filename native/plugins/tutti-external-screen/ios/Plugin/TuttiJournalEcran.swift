 ...
 */
final class TuttiJournalEcran {

    static let shared = TuttiJournalEcran()

    private let file = DispatchQueue(label: "app.tutti.journal.ecran", qos: .utility)
    ...
    func lancerSurveillance() {
        ...
        NotificationCenter.default.addObserver(forName: UIApplication.didReceiveMemoryWarningNotification, object: nil, queue: nil) { [weak self] _ in
            self?.note("systeme", "AVERTISSEMENT MÉMOIRE", ["residentMo": TuttiJournalEcran.memoireResidenteMo()], niveau: "warn")
        }
        NotificationCenter.default.addObserver(forName: UIApplication.willResignActiveNotification, object: nil, queue: nil) { [weak self] _ in
            self?.note("systeme", "app va passer inactive")
        }
        NotificationCenter.default.addObserver(forName: UIApplication.didBecomeActiveNotification, object: nil, queue: nil) { [weak self] _ in
            self?.note("systeme", "app active", ["residentMo": TuttiJournalEcran.memoireResidenteMo()])
        }
                    ...
                    if depuis >= 1.5 {
                        ...
                        if !enAttente || maintenant - dernierRapport >= 5 {
                            ...
                            self.note("surveillance", "FIL PRINCIPAL BLOQUÉ", [
                                ...
                                "residentMo": TuttiJournalEcran.memoireResidenteMo(),
                            ], niveau: "error")
                        }
                    }
                }
